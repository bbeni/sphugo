...
func NonPeriodic() {
	root := tg.MakeCellsUniform(220, tg.Vertical)
	...
	//tg.PlotBoundingCircles(canvas, root, 1, gx.WHITE)
	tg.PlotCells(canvas, root, 1, 1)
	...
	// Draw green circle
	radius := float32(math.Sqrt(p0.NNQueuePeekKey())*float64(w))
